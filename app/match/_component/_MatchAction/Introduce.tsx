import { reverseChangeOld } from "@/utils/dataFomat";
import styles from "./Introduce.module.scss"

export default function Introduce(props) {
    const { userInfo } = props;
    const { group_name, group_description, gender, age, member_count } = userInfo;
    const calcAge = reverseChangeOld(age)
    return (
        <div className={styles.box}>
            <div>이번에 소개할 팀은 <span className={styles.var}>{group_name}</span>입니다!</div>
            <div><span className={styles.var}>{calcAge}의 {gender} {member_count}</span>으로 이루어진,</div>
